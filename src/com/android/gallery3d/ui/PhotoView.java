...
/* Begin: added by yuanhuawei 20131111 */
import com.mediatek.common.featureoption.FeatureOption;
/* End: added by yuanhuawei 20131111 */
...
public class PhotoView extends GLView {
    ...
    private static final boolean CARD_EFFECT = false;//modified by lzp from true to false : 滑动方式改为平滑切换
    ...
    public void notifyDataChange(int[] fromIndex, int prevBound, int nextBound) {
        ...
        // Move the boxes
         /**Begin: modified by lzp on 20140308 for slide to camera **/
        /*mPositionController.moveBox(fromIndex, mPrevBound < 0, mNextBound > 0,
                mModel.isCamera(0), mSizes, mTypes);*/
        //zzd add 20140509 begin{
        if(mModel.isCamera(-1)){
        mPositionController.moveBox(fromIndex, mPrevBound < -1, mNextBound > 0,
                mModel.isCamera(0), mSizes, mTypes);
        }else{
	...
	//}zzd add 20140509 end
        /**End: modified by lzp on 20140308 for slide to camera**/
        for (int i = -SCREEN_NAIL_MAX; i <= SCREEN_NAIL_MAX; i++) {
            setPictureSize(i);
        }
    ...
    class FullPicture implements Picture {
	   ...
	   /* Begin: added by yuanhuawei 20131111 */
	   /*if(FeatureOption.MTK_LCA_RAM_OPTIMIZE){   //deleted by lzp
...
//        }
	...
	//zzd add 20140509 begin
	Log.d(TAG,"====>slideToPrevPicture");
	if(mModel.isCamera(-1)){
		if (mModel.getCurrentIndex()<=1) return false;
	}
	//zzd add 20140509 end
        if (mPrevBound >= 0) return false;//modified by lzp on 20140308 for slide to camera   if (mPrevBound >= 0)
    ...
    /* Begin: added by yuanhuawei for delete animation 20130909 */
    public void startDeleteAnimation(){
    	PositionController controller = mPositionController;
	Rect r = mPositionController.getPosition(0);
	int centerX = r.width()/2;
	int centerY = r.height()/2;
	//controller.zoomIn(centerX, centerY, 0.1f);
	controller.beginScale(centerX, centerY);
	int outOfRange = controller.scaleBy(0.4f, centerX, centerY);
    }

    public void endDeleteAnimation(){
	PositionController controller = mPositionController;
	controller.endScale();
    }
    /* End: added by yuanhuawei for delete anination 20130909 */
