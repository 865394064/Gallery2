...
public class LocalMergeAlbum extends MediaSet implements ContentListener {
    ...
    public ArrayList<MediaItem> getMediaItem(int start, int count) {
        ...
        for (int i = 0; i < size; i++) {
        	 if(mFetcher[i] != null)//added by lzp
        ...
        for (int i = markPos; i < start + count; i++) {
            ...
            subPos[k]++;
            if (i >= start) {
                if(slot[k] != null)//added by lzp
                	result.add(slot[k]);
            }
            if(mFetcher[k] != null)//added by lzp
    ...
    private static class FetchCache {
        ...
        public MediaItem getItem(int index) {
            ...
            return cache.get((index - mStartPos)<0?0:(index - mStartPos));//modified by lzp
        }
    }
